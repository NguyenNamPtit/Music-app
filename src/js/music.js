...
const loadApi = async () => {
  ...
  const slidersmall = $(".slider-music");
  const renderAlbums = (items) =>
    items
      .map(
        (item) => `
      <div class="item-music">
      <div class="item-musicx">
        <div>
        <img src="${item.img}" alt="" />
        <i class="bi bi-play-circle icon-play-music-box"></i>
      ...
      <div class="button-music">
        <div>
        <button class="button-music-play-all">Play All</button>
        <button class="button-icon">
          <a class="bi bi-suit-heart"></a>
        </button>
        </div>
      </div>
    </div>
  ...
  $(() => {
    slidersmall.html(renderAlbums(musicss));
    slidersmall.slick({
      autoplay: true,
      autoplaySpeed: 6000,
      infinite: true,
  ...
  $(() => {
    musicalbums.html(renderMusicAlbums(musicss.slice(0, 3)));
  });
  ...
  `
      ...
      .join("");
  $(() => {
    listAlbums.html(renderAlbumss(musicss));
    listAlbums.slick({
      autoplay: true,
      autoplaySpeed: 6000,
      infinite: true,
  ...
  slidersmall.on("click", "i", function () {
    const itemId = $(this).attr("data-id");
    currentSong = itemId;
    RenderSongPlay(currentSong);
    audio.src = musicss[currentSong].audio;
